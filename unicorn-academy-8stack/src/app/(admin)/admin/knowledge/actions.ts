...
export async function reindexDocument(docId: string): Promise<void> {
  ...
  if (doc.source_url) {
    ...
    // If text doc, we need the original text or chunks. But since pgvector chunks were deleted, we can fetch
    // the document and if we stored the raw text, we'd use it. But in schema, knowledge_docs doesn't have a 'raw_text' column.
    // In production, you would keep a raw text copy, but for now we throw error if url isn't present for reindex.
    throw new Error("Re-indexing is only supported for URL-source documents since raw text is not saved on the doc record.");
  }
