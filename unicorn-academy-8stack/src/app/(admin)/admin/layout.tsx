...
export default async function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="sidebar-layout">
      ...
      <aside className="sidebar">
        ...
        <div className="p-3 border-t border-border-muted">
          <div className="flex items-center gap-2 mb-3 px-2">
            <div className="w-7 h-7 rounded-full bg-bg-muted flex items-center justify-center text-xs border border-border-default">
              👤
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[8px] text-text-muted uppercase tracking-tighter">
                Admin Session
              </div>
            </div>
          </div>
          <form action="/auth/logout" method="POST">
            <button type="submit" className="w-full text-left nav-item text-red-600 hover:bg-red-50 hover:text-red-700">
      ...
      {/* ── Main ── */}
      <div className="main-content min-h-screen pl-[220px]">
        <div className="brand-accent-bar" />
        {children}
      </div>
    </div>
